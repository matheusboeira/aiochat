...
class WebSocket:
    ...
    async def send(self, message: str):
        ...
        formatted_message = f'{args.username}: {message}'
        data = self.websocket.send(wsproto.events.TextMessage(data=colored(formatted_message, "yellow")))
    ...
    def websocket_event_received(self, event: wsproto.events.Event) -> None:
        if isinstance(event, wsproto.events.TextMessage):
            if not event.data.__contains__(args.username):
